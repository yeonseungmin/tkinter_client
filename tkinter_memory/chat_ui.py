...
import tkinter as tk #AWT
from tkinter import ttk #ttk는 tkinter의 theme widget임 ,
...
import queue #FIFO 구조를 갖는 파이썬의 자료구조 중 하나 
...
class ChatUI:
    def __init__(self, root:tk.Tk):
        ...
        self.model_names: List[str]=[] #Ollama 서버에서 조회한 ai모델명을 담게될 리스트 
        self.is_busy=False #현재 AI가 응답 중인지 여부를 나타내는 플래그값, True인 경우 전송 버튼을 막아서 중복 전송 방지를 위함
        self.history: List[Message]=[] #대화 내역을 쌓아놓을 리스트
        self.worker_thread: threading.Thread | None=None #메인스레드 대신 업무를 수행할 별도 쓰레드
    ...
    #---------------------------------------------------------------------
    # 1.상단 영역 생성(ai모델 선택 드롭다운, 모델 새로고침 버튼, 대화을 싹 지워버리는 초기화 버튼)
    #---------------------------------------------------------------------
    ...
    def _build_chat_area(self)-> None:
        ...
        #wrap 줄바꿈 시 단어 단위로 감쌈 , state=tk.NORMAL 현재 편집이 가능한 상태임을 명시
        ...
        self._append_system_message(
            "프로그램이 시작되었습니다\n"
            "질문을 입력하시고 Enter 또는 전송 버튼을 누르세요.."
        )
    ...
    def _build_input_frame(self) -> None:
        ...
        input_frame.pack(fill=tk.X) #가로로 꽉 채우기..
    ...
    def _load_models(self) -> None:
        ...
        try:
            names=fetch_model_names() #Ollama에 설치되어 잇는 ai모델 담기 
            ...
            #상태표시줄에 실패 메시지 출력
            self.status_var.set("ai 모델 목록 조회 실패")

    #---------------------------------------------------------------------
    # 메시지 전송
    #---------------------------------------------------------------------
    def _get_input_text(self) -> str:
        # 1.0 은 첫번째 줄, 첫번째 문자 위치  
        # end-1c 맨 끝의 자동 개행 문자 1개를 제외한 실제 끝을 가져옴 
    ...
    def _send_message(self) -> None:
        ...
        #이미 처리 중이면 중복 전송을 막기 위해 바로 이 메서드 호출을 종료
        if self.is_busy:
            return
        ...
        self.worker_thread.start() #java 와 동일
    ...
    #2) 이벤트 처리
    #3) 애플리케이션 운영 
    # 결론) 절대로 무한루프, 네트워크 통신(응답 올때까지 대기에 빠지므로), 스트림처리 등등 대기상태에 빠지게 되는 업무는
    #         시키면 안됨!! (스마트폰에서는 컴파일 불가)
    def _worker_stream_chat(self, selected_model: str, user_message: str) -> None:

        # 대화 내역인 history를 다중 쓰레드 환경에서 바로 사용하게 되면 , 대화 내역이 꼬일 수있음
        # 전통적인 다중 쓰레드 환경에서는 원본을 바로 쓰지 않고, 복사본을 사용함 
        try:
            history_snapshot=list(self.history) 
            
            gen=stream_chat(model=selected_model, user_message=user_message, history=history_snapshot)
            print("gen 도달")

            for piece in gen:
                print("piece is "+piece)
                self.stream_queue.put(("piece", piece)) #() 튜플 : List와 흡사하지만, 안의 데이터를 수정불가
                

        except Exception as e:
            error_message=build_connection_error_message(e) #예외가 문자열로 반환
